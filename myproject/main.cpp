...
int main() {
    try {
        ...
        std::vector<DatasetConfig> pipeline = {
            // FASE 1: Dati di addestramento (Il materiale di studio per l'IA)
            {"example_images/", "features_cellule_train.csv", "ADDESTRAMENTO"},

            // FASE 2: Dati di test (L'esame finale, percorsi aggiornati)
            {"C:\\Progetti\\Template C++\\archive\\test\\img\\", "features_cellule_test.csv", "TEST BLINDATO"}
        };
        ...
        for (const auto& dataset : pipeline) {
            ...
            std::ofstream csvFile(dataset.outputCsv);
            if (!csvFile.is_open()) {
            ...
            for (size_t f = 0; f < imagePaths.size(); f++) {
                ...
                // DASHBOARD VISIVA
                cv::putText(imgAnteprima, "Fase: " + dataset.nomeFase, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
