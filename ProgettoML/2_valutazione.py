...
# Ignora i warning noiosi se manca una classe nel test set (es. il Rumore)
...
if __name__ == "__main__":
    ...
    # Salviamo sia la predizione secca che le probabilità
    ...
    # Estraiamo anche le feature scalate e le probabilità ESATTE solo per le ancore validate
    ...
    if len(y_true) > 0:
        ...
        classi_presenti = sorted(y_true.unique())
        disp = ConfusionMatrixDisplay(confusion_matrix(y_true, y_pred, labels=classi_presenti),
                                      display_labels=classi_presenti)
        ...
        # --- GRAFICO 2: PRECISION-RECALL CURVE (MODIFICATO) ---
        ...
        for i, nome_classe in enumerate(classi_str):
            # AGGIUNTA: Salta il Rumore se non vuoi vederlo nel grafico
    ...
    # =========================================================================
    # 6. VISUALIZZATORE OPENCV (PULITO E LEGGIBILE)
    # =========================================================================
    ...
    for img_name in immagini_da_mostrare:
        ...
        for _, row in img_data.iterrows():
            ...
            testo_vero = "?" if pd.isna(vera_label) else testi_brevi.get(vera_label, vera_label)

            if pred_label == 'Rumore':
                colore_ia = (128, 128, 128)
                spessore_box = 1
            ...
            (text_w, text_h), _ = cv2.getTextSize(testo_finale, font, 0.35, 1)

            cv2.rectangle(img_ia, (bx, by - text_h - 4), (bx + text_w, by), (0, 0, 0), -1)
            cv2.putText(img_ia, testo_finale, (bx, by - 2), font, 0.35, colore_ia, 1, cv2.LINE_AA)
