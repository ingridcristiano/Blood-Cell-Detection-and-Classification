...
def carica_e_valida_train():
    ...
    for img_name in immagini_uniche:
        ...
        # --- Mappiamo i confini sicuri dei Globuli Bianchi decisi dal medico ---
        box_bianchi_medico = []
...
def train_semi_supervised():
    ...
    print("--- 4. Addestramento XGBoost ---")
    # Calcoliamo dinamicamente i pesi per contrastare lo sbilanciamento del sangue
    pesi_classi = compute_sample_weight(class_weight='balanced', y=y_transduced)

    # Inizializziamo XGBoost per la classificazione multiclasse
    xgb_model = XGBClassifier(
        n_estimators=100,
        random_state=42,
        n_jobs=-1,
        objective='multi:softprob',  # Ottimizzato per output multiclasse
        eval_metric='mlogloss'
    )

    # Addestriamo passando i pesi per penalizzare gli errori sulle classi minoritarie
    xgb_model.fit(X_scaled, y_transduced, sample_weight=pesi_classi)
    ...
    # Salviamo il file come xgboost_model.pkl per non sovrascrivere la tua Random Forest
    joblib.dump(xgb_model, os.path.join(BASE_DIR, 'xgboost_model.pkl'))

    print("\n[FINE] Tutto completato! Modello XGBoost salvato.")
